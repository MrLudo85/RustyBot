...
bot.on('message', message => {
    if(message.content === prefix + "serverinfo"){
    ...
    }else if (message.coontent === prefix + "memberCount") {
        var embedmember = new Discord.RichEmbed()
        .setTitle("MemberCount")
        .addBlankField()
        .addField("member sur le serveur :", message.guild.memberCount)
        .setColor("#FF0105")
        message.channel.send(embedmember)
    }else if (message.content === prefix + "sondage") {
        if(message.author.id == "373616425727819776"){
            ...
            return message.reply("tu na pas la permission.")
        }
            ...
            if (message.member.hasPermission("MANAGE_MESSAGES")){
                message.channel.fetchMessage()
                        .then(function(list){
                            ...
                            message.channel.send("Je vien de supprimé des message")
